    ...
    <div className="card" style={{ borderColor: drag ? type.color : 'var(--border)', transition: 'border-color 0.2s' }}>
      ...
      <div
        ...
        style={{
          ...
          textAlign: 'center',
          cursor: 'pointer',
          background: drag ? `${type.color}11` : 'var(--bg3)',
          transition: 'all 0.2s'
        }}
      >
        <input
        ...
        {loading ? (
          <div style={{ color: 'var(--text2)' }}>
            <div style={{ fontSize: 22, display: 'inline-block' }}>⟳</div>
          ...
          <div style={{ color: 'var(--text2)' }}>
            ...
            <div style={{ fontSize: 12 }}>
              Glissez ou <span style={{ color: type.color, fontWeight: 600 }}>cliquez ici</span>
            </div>
      ...
      {result && (
        <div style={{ marginTop: 10, padding: '11px 13px', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.3)', borderRadius: 8 }}>
          <div style={{ color: 'var(--success)', fontWeight: 600, fontSize: 13, marginBottom: 3 }}>
            ✅ {result.nb_lignes ? result.nb_lignes.toLocaleString() : 0} lignes importées avec succès
          </div>
          {result.clean_stats && result.clean_stats.doublons_supprimes > 0 && (
            <div style={{ fontSize: 11, color: 'var(--text3)' }}>🗑️ {result.clean_stats.doublons_supprimes} doublons supprimés</div>
          )}
          {result.clean_stats && result.clean_stats.notes_imputees > 0 && (
  ...
  return (
    <div style={{ padding: 28 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 0, marginBottom: 24, flexWrap: 'wrap' }}>
        {['📥 Lecture', '✅ Validation', '🧹 Nettoyage', '🔄 Transformation', '💾 Stockage', '🔔 Alertes auto'].map((s, i, arr) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center' }}>
            <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 8, padding: '7px 11px', fontSize: 11, color: 'var(--text2)', whiteSpace: 'nowrap' }}>
              {s}
            </div>
            {i < arr.length - 1 && (
              <div style={{ width: 18, height: 1.5, background: 'linear-gradient(90deg,var(--accent),var(--accent2))', opacity: 0.5 }} />
            )}
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 24 }}>
        {TYPES.map(t => <DropZone key={t.id} type={t} onDone={reloadHistory} />)}
      </div>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        <div style={{ marginTop: 10, fontSize: 10, color: 'var(--text3)' }}>
          * Obligatoire. Le nettoyage automatique corrige les noms de colonnes similaires.
        </div>
      </div>
        ...
        {history.length === 0 ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead style={{ background: 'var(--bg3)' }}>
              <tr>
                {['Fichier', 'Type', 'Lignes', 'Statut', 'Date', 'Message'].map(h => (
                  <th key={h} style={{ padding: '9px 14px', textAlign: 'left', color: 'var(--text3)', fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 600 }}>
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {history.map((h, i) => (
                <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <td style={{ padding: '9px 14px', fontSize: 12, fontWeight: 600 }}>{h.nb_lignes ? h.nb_lignes.toLocaleString() : 0}</td>
                  <td style={{ padding: '9px 14px' }}>
                    <span style={{
                      fontSize: 10, padding: '3px 8px', borderRadius: 4,
                      background: h.statut === 'success' ? 'rgba(16,185,129,0.15)' : 'rgba(240,82,82,0.15)',
                      color: h.statut === 'success' ? 'var(--success)' : 'var(--danger)'
                    }}>
                      {h.statut}
                    </span>
                  </td>
                  <td style={{ padding: '9px 14px', fontSize: 10, color: 'var(--text3)', whiteSpace: 'nowrap' }}>
                    {h.created_at ? new Date(h.created_at).toLocaleString('fr-FR') : ''}
                  </td>
                  <td style={{ padding: '9px 14px', fontSize: 10, color: 'var(--text2)', maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {h.message}
                  </td>
                </tr>
              ))}
